...
from roocs_utils.project_utils import derive_ds_id, dsid_to_datapath
...
class DataBaseHandler(BaseHandler):
    ...
    def get_failed_runs(self):
        """
        :return: (dict) Dictionary of error types mapped to lists of job
        identifiers which result in them
        """
    ...
    def insert_success(self, identifier, content):
        """
        Inserts an entry into the table with a given identifier, the result 'success'
        and the content of the scan for this identifier
        :param identifier: (str) Identifier of the job
    ...
    def delete_by_dataset(self, ds_id, delete_errors=True):
        """
        Deletes entries specified by dataset id from the database
        :param identifier: (str) Identifier of the job
        :param delete_errors: (boolean) To specify whether error entries
        should be deleted for the dataset, default is True.
        """
        if delete_errors:
            query = f"DELETE FROM {self.table_name} WHERE id LIKE '{dsid_to_datapath(ds_id)}%';"
        else:
            query = f"DELETE FROM {self.table_name} WHERE id LIKE '{dsid_to_datapath(ds_id)}%' AND result='success';"
    ...
    def get_datasets_with_level(self):
        query = "SELECT (content ->> 'ds_id') FROM c3s_cmip6_catalog_results WHERE (content ->> 'level') <> ' ';"
