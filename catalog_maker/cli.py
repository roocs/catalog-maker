...
def _get_arg_parser_delete(parser):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Datasets to delete. Accepts comma separated"
        " list of datasets. E.g.: -d cmip6...gn.v20190910,cmip6...v20200202",
    )
    ...
    parser.add_argument(
        ...
        "--delete-errors",
        action="store_true",
        help="Delete the error entries for the dataset as well.",
    )
...
def parse_args_delete(args):
    ...
    ds_ids = args.datasets.split(",")
    delete_errors = args.delete_errors

    return project, ds_ids, delete_errors


def delete_main(args):
    project, ds_ids, delete_errors = parse_args_delete(args)

    rh = DataBaseHandler(table_name=f"{project.replace('-', '_')}_catalog_results")

    for ds_id in ds_ids:
        rh.delete_by_dataset(ds_id, delete_errors=delete_errors)
        if delete_errors:
            print(f"Deleted all entries for dataset: {ds_id}")
        else:
            print(f"Deleted all catalog entries for dataset: {ds_id}")
